...
const STATIC_SKILLS = [
  ...
  { label: 'PyCharm', icon: <SiPycharm className="text-[#21D789]" /> },
];
...
const TechStack = () => {
  ...
  const renderSkill = (skill: { label: string; icon: React.ReactElement }, index: number) => (
    <div
      key={index}
      className="mx-2 md:mx-3 flex items-center gap-2.5 md:gap-3 px-4 md:px-5 py-2.5 md:py-3 bg-background/40 backdrop-blur-sm border border-border/50 rounded-xl md:rounded-2xl shadow-sm hover:shadow-md hover:border-primary/30 hover:-translate-y-1 transition-all duration-500 group cursor-default"
    >
      <div className="text-xl md:text-2xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-3">
        {skill.icon}
      </div>
      <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] text-muted-foreground group-hover:text-foreground transition-colors">
        {skill.label}
      </span>
    </div>
  );
  ...
  return (
    <div className="w-full tech_stack_bg py-10 border-y border-border/10 relative overflow-hidden transition-all duration-700">
      <div className="flex flex-col lg:flex-row lg:items-center gap-6 lg:p-8 lg:gap-16">
        <div className="flex-shrink-0 flex items-center gap-5 group lg:rotate-[-90deg] lg:translate-x-[-20%]">
          <div className="hidden lg:block h-px w-8 bg-border/20 group-hover:w-16 group-hover:bg-primary transition-all duration-700" />
          <h6 className="font-black text-[10px] md:text-xs uppercase tracking-[0.5em] text-muted-foreground group-hover:text-foreground transition-colors whitespace-nowrap text-center">
            The Stack
          </h6>
          <div className="h-px w-8 bg-border/10 lg:hidden" />
        </div>

        <div className="flex-1 w-full overflow-hidden relative group/marquee space-y-4">
          <div className="absolute left-0 top-0 bottom-0 w-12 md:w-32 bg-gradient-to-r from-background via-background/20 to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-12 md:w-32 bg-gradient-to-l from-background via-background/20 to-transparent z-10 pointer-events-none" />
