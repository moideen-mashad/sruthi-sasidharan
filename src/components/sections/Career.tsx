      ...
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative mt-10">
            {/* Vertical line */}
            <div className="absolute left-[17px] top-0 bottom-0 w-0.5 bg-border/50" />

            <div className="space-y-8">
              {careers.map((career: CareerItem, index: number) => (
                <div key={career.id} className="relative pl-14 group">
                  {/* Icon bubble on the line */}
                  <div className="absolute left-0 top-5 w-[36px] h-[36px] rounded-full bg-foreground text-background flex items-center justify-center z-10 shadow-sm">
                    <LuBuilding2 size={16} />
                  </div>
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-5">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg lg:text-xl font-black tracking-tight text-foreground mb-1 leading-tight">
                          {career.role}
                        </h3>
                      ...
                      <div className="flex flex-wrap items-center gap-2 shrink-0">
                        <span className="px-2.5 py-1 bg-muted/30 border border-border rounded-lg text-[10px] font-black tracking-widest text-muted-foreground uppercase">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-border/50">
                      <button className="flex items-center gap-2 font-black text-[10px] uppercase tracking-widest group-hover:gap-3 transition-all text-foreground hover:text-primary">
                        Read Highlights <LuArrowRight size={13} className="text-primary" />
                      </button>
                      {career.location && (
          ...
          <div className="fixed inset-0 z-[9999]" role="dialog" aria-modal="true">
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4 sm:p-6" onClick={closeModal}>
              <motion.div
                ...
                className="relative w-full max-w-4xl bg-background rounded-[2rem] sm:rounded-[3rem] shadow-2xl border border-border max-h-[90vh] flex flex-col overflow-hidden"
              >
                <button
                  onClick={closeModal}
                  className="absolute top-6 right-6 sm:top-10 sm:right-10 z-50 p-3 bg-muted/80 backdrop-blur-md hover:bg-primary hover:text-primary-foreground rounded-full transition-all shadow-sm"
                >
                  <X size={24} />
                </button>
                ...
                <div className="flex-1 overflow-y-auto p-6 sm:p-10 lg:p-16 custom-scrollbar">
                  <div className="max-w-3xl">
                    <div className="mb-12">
                      <span className="inline-block px-4 py-1.5 bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-[0.2em] rounded-full mb-6">Experience</span>
                      <h3 className="text-3xl sm:text-4xl font-bold text-foreground mb-2 tracking-tight leading-none">{selectedCareer.role}</h3>
